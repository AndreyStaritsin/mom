...
function checkOrientation() {
  ...
  if (whatOrientation == "portrait-primary") {//если портретная
    rotate.style.display = "flex"//показываем rotate
    rotate.style.visibility = "visible"//делаем видимым
  }
  if (whatOrientation == "landscape-primary") {//если альбомная
...
window.onload = function () {//как все загрузится
...
play.onclick = function () {//при клике на play
...
for (let pismo = 0; pismo < pisma.length; pismo++) {
  pisma[pismo].addEventListener("click", function () {//вешаем слушателя клик циклом на письма
...
for (let convert = 0; convert < converts.length; convert++) {
  converts[convert].addEventListener("click", function () {
...
for (let i = 0; i < cards.length; i++) {
  cards[i].addEventListener("click", function (e) {
    ...
    if (e.target.localName == "img") { //если кликаем по фото
      ...
      if (!cards[i].classList.contains("card-active")) { //если у фото нет класса актив т.е неактивна
        if (sheet == sheetForSwipe) {
          ...
          showAlert("На этих страницах альбома нет места!<br> Сначала пролистай альбом!")
        }

        /*----------------------------------- ВЫКЛЮЧАЕМ ----------------------------------*/

      } else { //если у фото есть клас, т.е она активна
        if (cadr == 4 && sheet <= 34) { // и если счетчик мест в листе альбома равен 4 (т.е заполнен)
          albumWrap.insertAdjacentHTML("beforeend", //добавляем в конец лист в альбом с увеличенным номером листа и индексом 0
        ...
        if (numAddPhoto <= 138) { //если счетчик фото которые добавляем меньше их количества
          ...
          setTimeout(function () { //включаем фото и карточку
...
area.addEventListener("pointerdown", function (event) {//при нажатии
  start = {
    x: event.x,
    y: event.y
  }
})
area.addEventListener("pointerup", function (event) {//при конце нажатия поднятия пальца
  if (event)
    end = {
      x: event.x,
      y: event.y
  ...
  if (0 < way && way > (window.innerWidth / 3)) {//ЛИСТАНИЕ НАЗАД

    if (sheetForSwipe > 1) { //если количество листов в счетчике свайпа больше 1
      ...
      let previousSheet = document.querySelector(`.sheet-${previousSheetNumber}`)//предыдущий лист
      let currentLeftPage = document.querySelector(`.sheet-${currentSheetNumber}`).children[0] //левая страница текущего листа
      let currentRightPage = document.querySelector(`.sheet-${currentSheetNumber}`).children[1] //правая страница текущего листа
      let previousLeftPage = document.querySelector(`.sheet-${previousSheetNumber}`).children[0] //левая страница предыдущего листа
      let previousRightPage = document.querySelector(`.sheet-${previousSheetNumber}`).children[1] //правая страница предыдущего листа
  ...
  if (way < (window.innerWidth / 3 * -1) && way < 0) {//ЛИСТАНИЕ ВПЕРЕД
    if (sheetForSwipe < sheet) { //если кол-во листов в счетчике для свайпа < кол-ва листов общего
      let currentSheetNumber = sheetForSwipe //текущий номер листа
      ...
      let currentLeftPage = document.querySelector(`.sheet-${currentSheetNumber}`).children[0] //левая страница текущего листа
      let currentRightPage = document.querySelector(`.sheet-${currentSheetNumber}`).children[1] //правая страница текущего листа
      let nextLeftPage = document.querySelector(`.sheet-${nextSheetNumber}`).children[0] //левая страница предыдущего листа
      let nextRightPage = document.querySelector(`.sheet-${nextSheetNumber}`).children[1] //правая страница предыдущего листа
